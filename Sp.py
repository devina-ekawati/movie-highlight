...
from spacy.symbols import amod, NOUN
...
def get_highlight(doc):
	for possible_adjective in doc:
		#print possible_adjective.orth_, possible_adjective.dep_, possible_adjective.head.pos_
		if possible_adjective.dep == amod and possible_adjective.head.pos == NOUN:
			print possible_adjective.orth_, possible_adjective.head.orth_,

			if possible_adjective.head.head.pos == NOUN and possible_adjective.head.orth_ != possible_adjective.head.head.orth_:
				print possible_adjective.head.head.orth_

			print
			#verbs.add(possible_adjective.head)

nlp = English()

reviews = rt.getReview('Doctor Strange', 1, 'audiences')

for review in reviews:
	doc = nlp(unicode(review))
	get_highlight(doc)

reviews = mc.getReview('Doctor Strange', 1, 'audiences')

for review in reviews:
	doc = nlp(unicode(review))
	get_highlight(doc)

reviews = imdb.getReview('Doctor Strange', 0, 'audiences')
